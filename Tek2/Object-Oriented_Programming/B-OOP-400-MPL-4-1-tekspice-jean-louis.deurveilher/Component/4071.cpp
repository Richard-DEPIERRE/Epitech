...
void c4071::simulate(std::size_t tick)
{
    ...
    if (!_inputs.empty()) {
        ...
        _pins[2]->setState(getORState(_pins[0]->getState(), _pins[1]->getState()), tick);
        _pins[3]->setState(getORState(_pins[4]->getState(), _pins[5]->getState()), tick);
        _pins[9]->setState(getORState(_pins[7]->getState(), _pins[8]->getState()), tick);
        _pins[10]->setState(getORState(_pins[11]->getState(), _pins[12]->getState()), tick);
    }
}
