...
Droid::Droid(const Droid &new_droid): _serial(new_droid.getId()), _energy(new_droid.getEnergy()), _attack(new_droid.getAttack()),_toughness(new_droid.getToughness())
{
    _status = (new_droid._status) ? new std::string(*(new_droid.getStatus())) : new std::string("Standing by");
...
{
    return (new_droid.getEnergy() == _energy && *(new_droid.getStatus()) == *_status && new_droid.getId() == _serial);
}
